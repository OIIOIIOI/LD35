...
Game.prototype = $extend(openfl_display_Sprite.prototype,{
	reset: function() {
		...
		this.entities = [];
		this.spawnTick = this.spawnDelay = 30;
	...
	,spawnEnemy: function(playerOnTop) {
		...
		while(this.getDistanceXY(xx,yy,this.player.x,this.player.y) <= (function($this) {
			...
			xx = Std.random(Game.WIDTH - 2 * e.cx);
			yy = Std.random(Game.HEIGHT - 2 * e.cy);
		}
		e.x = xx;
		e.y = yy;
